...
tcell = read.csv('Data/D5 D28_booleans_AIM_with_media.csv',
                 check.names = F)%>%
...
write.csv(dfResult,'Result/A29_tfh_results_media.csv',row.names = F)
...
pdf('Result/A29_tfh_bar_GO_media.pdf',width = 5,height = 3)
plot(p)
dev.off()
...
p = ggplot(dfPlot,
           aes(x=t,y=reorder(gene,t)))+
  geom_bar(stat = 'identity')+theme_bw()
pdf('Result/A29_tfh_bar_olink_media.pdf',width = 3,height = 3)
plot(p)
dev.off()

p = ggplot(df1, aes(y=tcell,x = `olink_MCP-3_P80098_OID00474`))+
  geom_smooth(method = 'lm')+
  geom_point()+theme_bw()+facet_wrap(~Info_day)
pdf('Result/A29_tfh_MCP_media.pdf',width = 5,height = 3)
plot(p)
dev.off()
