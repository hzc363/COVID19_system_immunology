...
t1 = apply(RNAseq[,grepl('olink',colnames(RNAseq))], 2, var)%>%
  sort(decreasing = T)
top30 = c(names(t1)[1:30],'olink_DDX58_O95786_OID01018')

##### feature importance #####
yList = c("auc","mCD4","D28_IgG","M7_IgG","Info_severitycat")
xlist = c('Info_sex','Info_age',top30)

imp1 = data.frame()
for (i in 1:length(yList)) {
  dfTrain = RNAseq[,c(yList[i],xlist)]
  ...
  t1$y = yList[i]
  t1 = t1 %>%mutate(gini = gini/mean(gini))
  imp1 = imp1%>%rbind(t1)
}
imp1 = imp1%>%
  group_by(var)%>%
...
##### cross validation #####
predDf = expand.grid(1:length(xlist),1:nrow(RNAseq),yList)
