...
plot(log2(neut_ab$d28_IgG_AUC+1),log2(neut_ab$D28_Neut+1))
cor(log2(neut_ab$d28_IgG_AUC+1),log2(neut_ab$D28_Neut+1),use='complete')

plot(log2(neut_ab$M7_IgG_AUC+1),log2(neut_ab$M7_Neut+1))
cor(neut_ab$M7_IgG_AUC,neut_ab$M7_Neut,use='complete')
...
plotDf = df1%>%dplyr::select(Info_id,Info_tx,M7_IgG)%>%unique()
p = ggplot(plotDf,aes(x=Info_tx,y =M7_IgG ))+
  geom_boxplot()+theme_bw()
pdf('Result/A14_tx_vs_M7_IgG.pdf',3,3)
plot(p)
dev.off()

dfResult = data.frame()
for (i in grep('^GO_|^olink_|Gene_|EVE_',colnames(df1))) { 
  t1 = df1[,c("M7_IgG",'Info_onset','olink_DDX58_O95786_OID01018',
              colnames(df1)[i])]%>%na.omit()
  colnames(t1)[4]='y'
  LM1 = lm(M7_IgG~poly(Info_onset,2)+y,t1)
  LM1 = summary(LM1)
...
write.csv(dfResult,'Result/A15_Ab_M7_results_new.csv',row.names = F)
...
pdf('Result/A15_Ab_M7_bar_new.pdf',width =4,height = 3)
plot(p)
dev.off()
...
p = ggplot(dfPlot,
  ...
  geom_bar(stat = 'identity')+theme_bw()
pdf('Result/A15_Ab_M7_bar_new_rig.pdf',width =3,height =3)
plot(p)
dev.off()
...
p = ggplot(df1,aes(x=Info_severitycat,y=M7_IgG))+
  geom_boxplot()+theme_bw()+geom_jitter(width = 0.2)
pdf('Result/A15_Ab_vs_severity.pdf',width = 3,height = 3)
plot(p)
dev.off()

summary.aov(lm(M7_IgG ~ Info_severitycat, data = df1))


p = ggplot(df1,aes(x=D28_IgG,y=D28_IgG))+
  geom_point()+theme_bw()+geom_smooth(method = 'lm')
pdf('Result/A15_IgG_vs_IgG.pdf',width = 3,height = 3)
plot(p)
dev.off()
...
df1 = RNAseq%>%
  ...
  dplyr::select(Info_onset,DDX58,M7_IgG)%>%
  na.omit()%>%
  mutate(DDX58 = lm(DDX58~poly(Info_onset,2))$residuals)
p = ggplot(df1,aes(x=DDX58,y=M7_IgG))+
  geom_point()+geom_smooth(method = 'lm')+theme_bw()
pdf('Result/A15_DDX58_vs_M7_IgG.pdf',3,3)
plot(p)
dev.off()
...
df1 = RNAseq%>%
  ...
  filter(Info_severitycat!='Asymptomatic')%>%
  mutate(IL9=`EVE_IL-9`)%>%
  dplyr::select(Info_onset,IL9,M7_IgG)%>%
  na.omit()%>%
  mutate(IL9 = lm(IL9~poly(Info_onset,2))$residuals)
p = ggplot(df1,aes(x=IL9,y=M7_IgG))+
  geom_point()+geom_smooth(method = 'lm')+theme_bw()
pdf('Result/A15_IL9_vs_M7_IgG.pdf',3,3)
plot(p)
dev.off()

t1 = RNAseq[,c('M7_IgG','Info_tx')]%>%
  na.omit()%>%unique()%>%filter(is.finite(M7_IgG))
t.test(t1$M7_IgG~t1$Info_tx)
