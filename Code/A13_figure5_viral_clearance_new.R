...
p = ggplot(df1%>%filter(Info_day==0),aes(x=Info_onset,y=auc))+
...
p = ggplot(df1,aes(x=Info_severitycat,y=auc,color=Info_severitycat))+
  geom_boxplot()+geom_jitter(width =0.2)+theme_bw()
plot(p)
l1 = lm(auc~Info_severitycat,df1)
summary.aov(l1)
...
dfResult = data.frame()
for (i in grep('^GO_|^IgG_|^olink_|Gene_|EVE_',colnames(df1))) { #18:ncol(df1)
  t1 = df1[,c("auc",'Info_onset','olink_DDX58_O95786_OID01018',
              colnames(df1)[i])]%>%na.omit()
  colnames(t1)[4]='y'
  LM1 = lm(auc~poly(Info_onset,2)+y,t1)
  LM1 = summary(LM1)
...
write.csv(dfResult,'Result/A13_clearance_results_auc.csv',row.names = F)

dfResult = dfResult%>%
  filter(grepl('olink|EVE|GO_',gene))%>%
...
pdf('Result/A13_prime_bar_auc.pdf',width = 3,height = 3)
plot(p)
dev.off()
...
p = ggplot(dfPlot,
  ...
  geom_bar(stat = 'identity')+theme_bw()
pdf('Result/A13_prime_bar_auc_rig.pdf',width =3,height = 3)
plot(p)
dev.off()
...
p = ggplot(dfPlot,
  ...
  geom_point(aes(x=max(t)+1,color = pAdj>0.05),shape = 15)+
  geom_bar(stat = 'identity')+theme_bw()
pdf('Result/A13_prime_bar_auc_pathway.pdf',width = 5,height = 3)
plot(p)
dev.off()
...
##### severity vs AUC#####
df1 = RNAseq%>%dplyr::select(id, Info_severitycat,auc)%>%
  unique()
p = ggplot(df1,aes(x=Info_severitycat,y = auc))+
  geom_boxplot()+geom_jitter(width = 0.2)+theme_bw()
pdf('Result/A15_severity_vs_auc.pdf',3,3)
plot(p)
dev.off()
summary.aov(lm(auc~Info_severitycat,df1))
##### DDX58 vs auc #####
df1 = RNAseq%>%
  mutate(Info_onset = Info_day-Info_onset)%>%
  filter(Info_severitycat!='Asymptomatic')%>%
  mutate(DDX58=olink_DDX58_O95786_OID01018)%>%
  dplyr::select(Info_onset,DDX58,auc)%>%
  na.omit()%>%
  mutate(DDX58 = lm(DDX58~poly(Info_onset,2))$residuals)
p = ggplot(df1,aes(x=DDX58,y=auc))+
  geom_point()+geom_smooth(method = 'lm')+theme_bw()
pdf('Result/A15_DDX58_vs_auc.pdf',3,3)
plot(p)
dev.off()
...
df1 = RNAseq%>%
  ...
  filter(Info_severitycat!='Asymptomatic')%>%
  mutate(IL9=`EVE_IL-9`)%>%
  dplyr::select(Info_onset,IL9,auc)%>%
  na.omit()%>%
  mutate(IL9 = lm(IL9~poly(Info_onset,2))$residuals)
p = ggplot(df1,aes(x=IL9,y=auc))+
  geom_point()+geom_smooth(method = 'lm')+theme_bw()
pdf('Result/A15_IL9_vs_auc.pdf',3,3)
plot(p)
dev.off()
