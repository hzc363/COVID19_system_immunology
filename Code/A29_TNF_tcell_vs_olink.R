...
p = ggplot(dfPlot,aes(x=Info_onset+28,y=tcell))+
  ...
  ylab('IFNg-IL21-TNF+ response')+
  xlab('Time since symptom onset')+
  theme_bw()
pdf("Result/A29_onset_vs_TNFa_T_cell.pdf",5,5)
plot(p)
dev.off()
...
write.csv(dfResult,'Result/A29_t_cell_results.csv',row.names = F)

dfResult = dfResult%>%
  filter(grepl('olink_|GO_',gene))%>%
  ...
  select(-nc)%>%
  arrange(p)
write.csv(dfResult,'Result/A29_TNF_Tcell_results.csv',row.names = F)
...
pdf('Result/A29_t_cell_bar_GO.pdf',width = 5,height = 3)
plot(p)
dev.off()
...
p = ggplot(dfPlot,
           aes(x=t,y=reorder(gene,t)))+
  geom_bar(stat = 'identity')+theme_bw()
pdf('Result/A29_t_cell_bar_olink.pdf',width = 3,height = 3)
plot(p)
dev.off()
...
pdf('Result/A29_tcell_MCP.pdf',width = 5,height = 3)
plot(p)
dev.off()
