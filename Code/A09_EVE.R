...
##### prepare data #####
RNAseq = cbind('sample_id'=paste0('S',1:nrow(eve1)),eve1)

##### correlation between EVE and olink #####
t1 = olink%>%gather(key="protein",value = 'olink_value',-id, -day)
t2 = eve1%>%gather(key="protein",value = 'eve_value',-id, -day)
t1 = t1%>%
...
t2 = t2%>%
  ...
  inner_join(t1,by=c("id", "day", "protein"))%>%
  mutate(eve_value = as.numeric(eve_value))%>%
  mutate(eve_value = log2(eve_value))

p = ggplot(t2,aes(x=olink_value,y =eve_value))+
  geom_point()+theme_bw()+
...
RNAseq[,-c(1:16)] = RNAseq[,-c(1:16)]%>%apply(2,as.numeric)%>%apply(2,log2)
##### PCA of samples ####
#RNAseq = RNAseq%>%filter(sample_id!='S120')
#info1 = info1%>%filter(sample_id!='S120')

expr1 = t(RNAseq[,-c(1:16)])
...
p = ggplot(df1,aes(x=`IL-18`,y = antibody_value))+
...
pdf('Result/A09_PCA.pdf',width = 15, height = 7)
plot(p)
dev.off()
...
##### save #####
write.csv(df_result,'Result/A09_tcell_result.csv',row.names = F)
